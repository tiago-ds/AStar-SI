class Graph {
	...
	getAdjacentEdges(vertexName) {
		return this.edgeMap.get(vertexName).filter((edge) => {
			if (edge.realDistance != null) return edge;
		});
	}

	getEdge(vertexOrigin, vertexDestiny) {
		return this.edgeMap.get(vertexOrigin).filter((edge) => {
			if (edge.vertexDestiny == vertexDestiny) return edge;
		});
	}
	...
	addEdge(vertexOrigin, vertexDestiny, realDistance, directDistance, color) {
		if (
			!this.vertexMap.has(vertexOrigin) &&
			!this.vertexMap.has(vertexDestiny)
		)
			return;
		let origin = this.getVertex(vertexOrigin);
		let destiny = this.getVertex(vertexDestiny);

		var originToDestiny = new Edge(
			vertexOrigin,
			vertexDestiny,
			origin,
			destiny,
		...
		if (vertexOrigin === vertexDestiny) return 0;
			...
			for (const edge of this.getVertexEdges(vertexOrigin)) {
				if (edge.vertexDestiny == vertexDestiny) {
					realDistance = (edge.realDistance/30)*60;
				}
			}
			return realDistance;
		}
	...
	getDirectDistance(vertexOrigin, vertexDestiny) {
		if (vertexOrigin === vertexDestiny) return 0;
			...
			for (const edge of this.getVertexEdges(vertexOrigin)) {
				if (edge.vertexDestiny == vertexDestiny) {
					directDistance = (edge.directDistance/30)*60;
				}
			}
	...
	getRealEdgeColor(vertexOrigin, vertexDestiny) {
		let edges = this.edgeMap.get(vertexOrigin).filter((edge) => {
			if (
				edge.realDistance != null &&
				edge.vertexDestiny == vertexDestiny
			)
				return edge;
		});
